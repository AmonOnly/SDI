    ...
    while not stop_event.is_set():
        try:
            ...
            if msg_type == "prepare":
                ...
                # CORREÇÃO: Converter string para tupla para comparação correta
                proposal_id_tuple = parse_proposal_id(proposal_id_str)

                with lock_acceptor:
                    promised_id_tuple = parse_proposal_id(acceptor_state["promised_id"]) if acceptor_state["promised_id"] else None
                    
                    if promised_id_tuple is None or proposal_id_tuple >= promised_id_tuple:
                        acceptor_state["promised_id"] = proposal_id_str
                        response = {
                            "ok": True,
                            "accepted_id": acceptor_state["accepted_id"],
                            "accepted_value": acceptor_state["accepted_value"]
                        }
                    else:
                ...
                with lock_acceptor:
                    ...
                    if promised_id_tuple is None or proposal_id_tuple >= promised_id_tuple:
                        acceptor_state["accepted_id"] = proposal_id_str
                        acceptor_state["accepted_value"] = value
                        acceptor_state["promised_id"] = proposal_id_str # Proposta aceita se torna a última prometida
                        response = {"ok": True}
                    else:
                        response = {"ok": False, "reason": f"Promised {promised_id_tuple} already"}
                conn.sendall(json.dumps(response).encode())
